...
namespace Makaretu.Dns.Resolving
{
    ...
    public class Catalog : ConcurrentDictionary<DomainName, Node>
    {
        ...
        {
            UnityEngine.TextAsset rootHints = UnityEngine.Resources.Load<UnityEngine.TextAsset>("ROOT_HINTS");
            if (rootHints == null)
            {
                UnityEngine.Debug.LogError("RootHints resource not found.");
                return null;
            }

            using (var reader = new StringReader(rootHints.text))
            {
                var presentationReader = new PresentationReader(reader);
                ResourceRecord r;
                while (null != (r = presentationReader.ReadResourceRecord()))
                {
                    Add(r);
                }
            }
